...
| I-032 | Modal sheet sat behind the iOS keyboard | high | resolved | 2026-08-21 | Reported from a phone: "tried to add contact and the form wasn't in focus mode and overlayed with keyboard." Reproduced by measurement — the sheet occupied y=445..844 while an iOS keyboard covers y=508..844, so **all three inputs (563/639/715) were underneath it**, and `document.activeElement` was `body` so nothing was focused either. Cause: `max-h-[90vh]` — `vh` does not shrink when iOS raises the keyboard; only the *visual* viewport does, so a bottom-anchored sheet stays exactly where it was. **Resolved:** `Modal` grew a `useKeyboardInset` hook reading `visualViewport.height/offsetTop`, lifting the sheet by the covered strip and capping height to what is visible; plus autofocus of the first field and `scrollIntoView({block:'center'})` on focusin. Verified by shrinking `visualViewport` 336 px in-page: sheet bottom lands exactly on the keyboard top and `inputsUnderKeyboard: 0`. |
| I-033 | First mobile pass missed non-visible defects | medium | resolved | 2026-08-21 | The v1 responsive sweep only measured pages in their **closed** state, so nothing inside a modal was ever checked, and it measured only overflow — not touch size, font size, or accessible names. A second audit across all 10 surfaces *with modals open* found: Templates' reorder chevrons at **14×14 px** (my earlier patch matched a different class string and skipped them), 48 unlabelled icon buttons, the CSV textarea still at 12 px (the iOS-zoom rule listed `.text-sm` explicitly and the textarea is `.text-xs`), and Preferences switches stretched to **332 px wide** by a blanket `[&>*]:w-full` I had added. All fixed; audit now returns 0 issues on all 10 surfaces. **Rule: audit interactive states, not just resting ones, and measure properties a screenshot cannot show.** |
