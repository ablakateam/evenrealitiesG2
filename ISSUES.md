...
| I-007 | Text-drawn box frames can't align on the G2 font | medium | open | — | 2026-05-14 | P11 simulator testing: the G2 firmware font renders box-drawing glyphs (`─│┌┐└┘`) and letters at DIFFERENT advance widths — a row of 37 dashes spans ~full container width while 37 letters span ~half. So the planned Pine/Norton-Commander frame, drawn as box chars in text `content`, can't keep its right edge aligned. **P12 fix:** redesign the HUD visual system to use the SDK's real container `borderWidth` / `borderColor` / `borderRadius` for framing, with text `content` carrying only inner lines. Frame helpers in `hud/src/render.ts` are kept but marked provisional; P11 pages use plain left-aligned text. |
