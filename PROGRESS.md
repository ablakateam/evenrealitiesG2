...
| 0.1.17 | `e24df34` | 2026-08-20 | **Launch behaviour:** Router input-settle window (700 ms) drops the temple tap that launches the app, so it can't bleed into the first mounted page; Idle rebuilt as a 4-row hub (Speak · Inbox · Voice command · Style) instead of one full-screen "tap to speak" capture surface. **Message style:** corrected `LIST_ROW_PITCH` 32 → 40 px (measured off rendered baselines) — Confirm's third row was never being drawn, which is why style looked locked to Casual; ready list is now a guaranteed-visible 2 rows (SEND / `Style: X >`) with a scrollable submenu. **Reachability:** Inbox + Inbox-read migrated to the chrome container shape and wired to the hub (they carried a pre-chrome `{1,2,3}` shape that would have tripped L:38); VoicePage reachable again. **Consistency:** HUD reads/writes `/api/config`, so `default_tone` is shared by the glasses Style menu, a new draft's starting tone, and the companion's new style card. **Correctness:** outbound rewrites sanitized for the G2 font (I-003); packed builds no longer substitute a canned transcription when the mic returns nothing; explicit `AudioInputSource.Glasses`; single-sourced version; 4 orphaned picker pages deleted (390 LOC). SDK 0.0.13 → 0.0.14, CLI → 0.1.14, simulator → 0.9.0 | Hardware v0.1.16 test |

| 0.1.18 | `725e935` | 2026-08-20 | **Glasses visuals:** Idle "moving stars" replaced with a radiating voice signal `( ( ( ● ) ) )` that breathes outward — one line, reads as sound leaving a source rather than decoration. Recording waveform rebuilt from 16 block glyphs (`▁▂▃▄▅▆▇█`, "Minecraft") into a 4-row ASCII oscilloscope trace: thin dash marks plotted at amplitude height, with exponential smoothing and a perceptual (sqrt) RMS curve so ordinary speech uses the full range instead of hugging the baseline. **STT language bug:** English speech transcribed as Japanese — `voice_language` was `'auto'`, so `whisper-1` was free to pick, and on marginal audio it hallucinates in another language. Pinned language (migration 2 moves existing rows `auto → en`), bounded the Whisper bias prompt (was every contact name — unbounded, itself a hallucination trigger), added a server-side silence guard that returns 422 `silent_audio` instead of paying for a hallucinated transcript, and logging of requested-vs-detected language + RMS on every call. | Hardware v0.1.17 test |

| 0.1.19 | *(pending)* | 2026-08-20 | **Passkey-free connect.** Opening the dashboard from the companion no longer means typing a 32-char secret, and the Account QR no longer embeds the permanent one. New `auth_handoffs` table + `POST /api/auth/handoff` / `…/exchange`: a client that already holds the secret mints a single-use, 180 s token; only its SHA-256 is stored, and the secret rides on the row AES-GCM encrypted (captured from the Bearer header at mint, so it survives rotation). Companion gains an "Open dashboard" button that deep-links `/connect?t=…`, plus a masked passkey with Show. Dashboard gains a `/connect` route that exchanges once and strips the token from history. 8 new tests: single-use, expiry, unknown token, no plaintext at rest, and two racing exchanges. | Hardware v0.1.18 feedback |
